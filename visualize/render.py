...
if __name__ == "__main__":
    main()"""
Render a saved weight vector in MuJoCo and save as MP4 + GIF.

Trajectory-agnostic: works with any trajectory generator that implements
generate_trajectory(weights).
"""
...
def render_rollout(env, policy, weights, sim_steps, reward_fn, termination_fn,
                   reward_cfg, width=1280, height=720, camera_name=None):
    """
    Run a rollout and capture frames from MuJoCo's offscreen renderer.
...
def run_render(trajectory_cls, config, weights_path=None, sim_steps=None,
               fps=30, width=1280, height=720, camera=None,
               output_dir=None, no_gif=False):
    """
    Render a rollout and save as MP4 + GIF.

    Parameters
    ----------
    trajectory_cls : class
        Trajectory generator class.
    config : dict
        Full config dict.
    weights_path : str or None
        Path to weights .npy file. Default: best_weights.npy in logdir.
    sim_steps : int or None
        Simulation steps. Default: from config.
    fps : int
        Frames per second for video output.
    width, height : int
        Video resolution.
    camera : str or None
        MuJoCo camera name. None = free camera.
    output_dir : str or None
        Output directory. Default: logdir/renders.
    no_gif : bool
        Skip GIF generation.
    """
    env_cfg = config["env"]
    ...
    logdir = train_cfg["logdir"]
    sim_steps = sim_steps or train_cfg["sim_steps"]
    ...
    # Output directory
    output_dir = output_dir or os.path.join(logdir, "renders")
    os.makedirs(output_dir, exist_ok=True)

    # Build env and policy
    env = Go2CrawlEnv(**env_cfg)
    policy = trajectory_cls(**policy_cfg)
    reward_fn = get_reward_fn(reward_cfg["reward_fn"])
    termination_fn = get_termination_fn(reward_cfg["termination_fn"])
    ...
    dt = env.model.opt.timestep * env_cfg["control_substeps"]
    sim_fps = 1.0 / dt
    skip = max(1, int(sim_fps / fps))
    frames_out = frames[::skip]
    print(f"  Sim FPS:    {sim_fps:.0f}")
    print(f"  Output FPS: {fps} (every {skip} frames -> {len(frames_out)} frames)")

    # Save MP4
    mp4_path = os.path.join(output_dir, "rollout.mp4")
    imageio.mimsave(mp4_path, frames_out, fps=fps, codec="libx264")
    print(f"Saved: {mp4_path}")

    # Save GIF
    if not no_gif:
        gif_path = os.path.join(output_dir, "rollout.gif")
        gif_frames = [frame[::2, ::2] for frame in frames_out]
