...
#         # for j in range(12):
#         #     current_pos = data.qpos[7 + j]
#         #     current_vel = data.qvel[6 + j]
#         #     data.ctrl[j] = kp * (target_qpos[j] - current_pos) + kd * (0.0 - current_vel)
#         mujoco.mj_step(model, data)
#         viewer.sync()

#         current = np.round(data.qpos[7:19],3)
#         current_tuple = tuple(current)
#         if last_printed is None or current_tuple != last_printed:
#             print("joint Angles:" , current.tolist())
#             laspt_printed = current_tuple
