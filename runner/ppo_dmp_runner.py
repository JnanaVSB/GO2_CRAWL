...
def run_training_loop(
    ...
    agent : PPODMPAgent
        PPO agent with get_action_value_logprob / add_to_rollout_buffer /
        compute_returns_and_train interface.
    total_episodes : int
    ...
    try:
        for ep in range(start_episode, total_episodes):
            ...
            # 1. Get action + value + log_prob from PPO policy
            obs = agent.get_obs()
            ...
            # 5. Train if buffer is full (every n_steps episodes)
            did_update = False
            if agent.is_buffer_full():
                agent.compute_returns_and_train()
                num_updates += 1
                did_update = True
